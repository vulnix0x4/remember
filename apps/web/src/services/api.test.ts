...
describe("fixture-backed API boundary", () => {
  ...
  it("maps a POST response and sends a public timestamped transcript with the raw reaction", async () => {
    ...
    const local = { ...imprints[0], personalReaction: "The raw note", hypothesis: "An AI guess" };
  ...
  it("maps semantic search and grounded Ask API responses", async () => {
    ...
    const fetcher = vi.fn<typeof fetch>()
      ...
      .mockResolvedValueOnce(new Response(JSON.stringify({ answer: "Grounded answer", grounded: true, limitations: ["Saved sources only."], citations: [{ itemId: "search-id", title: "Result", timestampSeconds: 42, url: "https://example.com#moment" }] }), { status: 200 }));
