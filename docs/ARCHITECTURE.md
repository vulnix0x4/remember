# Remember private app architecture
...
iOS + HealthKit + EventKit    Web/PWA     Browser Extension
             ...
             D1             Workflow jobs          R2
              |                  |                  |
        personal records    source analysis    files + exports
              |                  |
              +----------- Vectorize ---------+
```
...
- AI answers can reference only retrieved, user-owned saved items and must emit structured citations.
...
- Every personal-record table carries `user_id`; all reads, mutations, downloads, and R2 keys enforce that ownership boundary.
...
| Library and Ask | D1 + Vectorize projection | Answers cite user-owned sources |
| Goals | D1 | A goal describes an observable result |
| Tasks | D1 | At most one active task per user |
| Daily basics | D1 | A completion is an explicit date, not an inferred streak |
...
Task handling is intentionally adaptive. Completing the current task selects the best queued task. Marking a task blocked records the reason and either shrinks, clarifies, time-boxes, adapts, or removes it. The database partial index enforces the single-active invariant even if multiple clients race.
...
8. Compare against previous saves and store typed, explainable connections.
9. Update source-backed topics, takeaways, contrasts, history, and resurfacing candidates.
...
- Export reads from D1 and produces versioned portable JSON plus Markdown for both saved knowledge and personal records. File contents remain individually downloadable instead of being silently duplicated into an export.
